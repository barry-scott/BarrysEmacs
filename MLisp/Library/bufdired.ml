...
(defun
    ~BUF-previous-line()
    ; make sure that we are not in the header area
    (if (> (current-line-number) 3)
        (previous-line)
        (goto-line 3)
    )
)
...
(defun
    ~BUF-get-from-library()
    ~bufdir-dot ~local-dot ~buffer-name
    (setq ~bufdir-dot (+ 0 (dot)))
    (save-window-excursion
    ...
    (~BUF-locate-buffer)
    (goto-character ~bufdir-dot) (beginning-of-line)
    (message (concat "Fetched " ~buffer-name " from the MLisp-library"))
)
(defun
    ~BUF-put-in-library()
    ~bufdir-dot ~buffer-name
    (setq ~bufdir-dot (+ 0 (dot)))
    (save-window-excursion
        (temp-use-buffer (~BUF-get-buffer-name))
        (setq ~buffer-name (current-buffer-name))
    ...
    (~BUF-locate-buffer)
    (goto-character ~bufdir-dot) (beginning-of-line)
    (message (concat "Put " ~buffer-name " into the MLisp-library"))
)
(defun
    ~BUF-visit-file()
    ~file-name ~error-found
    (save-window-excursion
...
(defun
    ~BUF-write-file()
    ~bufdir-dot ~buffer-name ~file-name
    (setq ~bufdir-dot (+ 0 (dot)))
    (save-window-excursion
        (temp-use-buffer (~BUF-get-buffer-name))
        (write-current-file)
    ...
    (~BUF-locate-buffer)
    (goto-character ~bufdir-dot) (beginning-of-line)
    (message (concat "Wrote buffer " ~buffer-name " to " ~file-name))
)
(defun
    ~BUF-get-buffer-name()
    ~buffer-name
    (save-window-excursion
        (if (error-occurred
            (setq ~buffer-name
                (fetch-array buffer-names (- (current-line-number) 2))))
...
(defun
    ...
    (~BUF-locate-buffer)
    (goto-character ~bufdir-dot)
    ; handle positioning at the end of the list
    ; after deleting the last buffer in the list
    (previous-line)
    (~BUF-next-line)
)
