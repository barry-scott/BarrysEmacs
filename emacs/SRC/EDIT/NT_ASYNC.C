    ...
    )
{
    ...
    MyOverlapped = (PMYOVERLAPPED)malloc(sizeof(MYOVERLAPPED), malloc_type_char);
    ...
    if (MyOverlapped->Overlapped.hEvent == NULL) {
        DebugPrintf("CreateAsync failed to create event, error = %d\n", GetLastError());
        free(MyOverlapped);
        return(NULL);
    }
    ...
    )
{
    ...
    if (!Result) {
        DebugPrintf("ReadFileAsync : Failed to reset async event, error = %d\n", GetLastError());
        return(FALSE);
    }
    ...
    if (!Result) {
        ...
        DebugPrintf("ReadFileAsync : ReadFile failed, error = %d\n", Error);
    ...
    if (!Result) {
        DebugPrintf("ReadFileAsync : Failed to set async event, error = %d\n", GetLastError());
    }
    ...
    )
{
    ...
    Result = ResetEvent(MyOverlapped->Overlapped.hEvent);
    if (!Result) {
        DebugPrintf("WriteFileAsync : Failed to reset async event, error = %d\n", GetLastError());
        return(FALSE);
    }
    ...
    if (!Result) {
        ...
        DebugPrintf("WriteFileAsync : WriteFile failed, error = %d\n", Error);
        MyOverlapped->FileHandle = NULL;
        return(FALSE);
    }
    ...
    Result = SetEvent(MyOverlapped->Overlapped.hEvent);
    if (!Result) {
        DebugPrintf("WriteFileAsync : Failed to set async event, error = %d\n", GetLastError());
    }
    ...
    )
{
    ...
    if (WaitResult != 0) {
        DebugPrintf("GetAsyncResult : Event was not set, wait result = %d\n", WaitResult);
    ...
    if (MyOverlapped->CompletedSynchronously) {
        ...
        *BytesTransferred = MyOverlapped->BytesTransferred;
        ...
        if (!Result) {
            ...
            DebugPrintf("GetAsyncResult : GetOverlappedResult failed, error = %d\n", AsyncResult);
        }
    }
