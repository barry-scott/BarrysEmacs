...
/*forward*/ char * clisetstr( void );
/*forward*/ static void mail_box_AST_routine( struct channel *chan );
/*forward*/ static void termination_AST( struct process_blk *dead_process );
/*forward*/ int process_channel_interrupts( void );
/*forward*/ static void close_channel( struct channel *chan );
/*forward*/ static int setup_channel( struct channel *chan );
...
/*forward*/ int associate_mbx( void );
/*forward*/ int deassign_mbx( void );
/*forward*/ void reassign_mbx( void );
#endif
...
#if BROADCAST
struct process_blk *broad_cast_process = 0;
#endif
 ...
 */
static void mail_box_AST_routine( struct channel *chan )
	{
	/*
 ...
 *
 */
static void termination_AST( struct process_blk *dead_process )
	{
		...
		if( chan->chan_interrupt )
			...
			if( chan->chan_read_channel )
				...
				while( input_pending == 0 && loop_count > 0 )
					 ...
					 */
					if( ! VMS_SUCCESS( status ) ) invoke_debug();
				...
				if( ! VMS_SUCCESS(status) ) invoke_debug();
				if( ! VMS_SUCCESS(iosb[0]) ) invoke_debug();
 ...
 *
 */
static void close_channel( struct channel *chan )
	{
	/*
 ...
 *
 */
static int setup_channel( struct channel *chan )
	{
	int code;
	int status;
	unsigned short int iosb[4];
...
int start_DCL_process( void )
	...
	if( brd_cst_mbx && broad_cast_process != 0 )
		{
		error( u_str("duplicate broadcast process.") );
		return 0;
		}
#endif
	for( i=0; i<n_processes; i++ )
	...
	if( setup_channel( &proc->proc_input_channel ) != 0 )
		{
		error( u_str("could not create input channel.") );
		return 0;
		}
#if BROADCAST
	if( ! brd_cst_mbx )
		...
		if( setup_channel( &proc->proc_output_channel ) != 0 )
			{
			error( u_str("could not create output channel.") );
			return 0;
			}
	/*
	...
	if( ! VMS_SUCCESS(_spawn
		...
		error( u_str("could not spawn subprocess.") );
		return 0;
		}
	/*
	 *	This is now the current process
...
int associate_mbx( void )
	...
	if( (get_log(u_str("TT"), term_name_text) ) == 0 )
		{
		error( u_str("Could not setup terminal.") );
		return 0;
		}
...
void init_vms( void )
	...
	if( process_ptrs == NULL )
		{
		process_ptrs = malloc( n_process_slots * sizeof( struct process_blk * ), malloc_type_star_star );
		process_ptrs[0] = 0;
		}
	...
	for( i=0; i<n_processes; i++ )
		...
		if( proc->proc_input_channel.chan_brdcst_chan  )
			...
			if( ! (_spawn
				...
				{
				termination_AST( proc );
				error( u_str("could not spawn sub-process.") );
				break;
				}
			}
		}
