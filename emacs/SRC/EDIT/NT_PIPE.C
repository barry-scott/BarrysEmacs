    ...
    )
{
    ...
    sprintfl( u_str(PipeName), sizeof(PipeName), u_str(PIPE_FORMAT_STRING), GetCurrentProcessId(), PipeSerialNumber++);
    ...
    *ReadHandle = CreateNamedPipeA(
    ...
    if (*ReadHandle == NULL) {
        DebugPrintf("MyCreatePipe: failed to created pipe <%s>, error = %d\n", PipeName, GetLastError());
        return(FALSE);
    }
    ...
    *WriteHandle = CreateFileA(
    ...
    if (*WriteHandle == INVALID_HANDLE_VALUE ) {
        DebugPrintf("Failed to open client end of pipe <%s>, error = %d\n", PipeName, GetLastError());
