...
#define STR( name, field, value ) \
    ...
    // setup the clipboard
    const char *clipboard_name = getenv( "BEMACS_CLIPBOARD" );
    if( clipboard_name == NULL )
        default_clipboard = XA_CLIPBOARD( dpy );
    else
        default_clipboard = clipboardType( clipboard_name );

    Widget shell_widget = XtAppCreateShell( "BEmacs", emacs_class, applicationShellWidgetClass, dpy, NULL, 0 );
...
Atom EmacsMotif_Application::clipboardType( const EmacsString &name )
{
    if( name == "primary" )
        return XA_PRIMARY;
