...
/*forward*/ bool send_exit_message( const EmacsString &str );
#endif
/*forward*/ int debug_emacs( void );
/*forward*/ int argc_command( void );
/*forward*/ int argv_command( void );
/*forward*/ int argIsQualifier_command( void );
/*forward*/ int emacs_version( void );
/*forward*/ int emacsMain( const EmacsString &rest_fn, const EmacsString &device, const EmacsString &term_type );
static void read_emacs_memory_file();
static void write_emacs_memory_file();
/*forward*/ int execute_package( const EmacsString &package );
/*forward*/ int read_in_files(void);
...
static void write_emacs_memory_file()
	...
	while( w != 0 )
		...
		if( !bf_cur->b_fname.isNull() )
			...
			if( !args.fio_is_open() )
				{
				args.fio_create( memory, 1, FIO_STD, EmacsString(), (FIO_RMS_Attribute)(int)default_rms_attribute );
				if( !args.fio_is_open() )
					break;
				}
