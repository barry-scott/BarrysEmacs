 ...
 * TABLE OF CONTENTS:
 */
/*forward*/ int getword( EmacsString * *table_ref, const EmacsString &s );	/* fetch a completed word */
/*forward*/ static EmacsString getnsword( EmacsString * *table, EmacsString ** (*table_maker)( const EmacsString &, int * ), 
	const EmacsString & );	/* fetch a non-space fill word */
/*forward*/ EmacsString getescword( EmacsString * *table_ref, const EmacsString &s );	/* fetch an escape expanded word */
/*forward*/ EmacsString getescfile( const EmacsString fmt, va_list *argp, int dirs, int vers );		/* fetch an expanded filename */
/*forward*/ static EmacsString ** get_file_list( EmacsString file, int *dir_spec_len );	/* Make a filename table */
/*forward*/ EmacsString getescdb( struct dbsearch *current_dbs, const EmacsString fmt, ... );	/* fetch an expanded DB searchlist key */
#ifdef DB
/*forward*/ static EmacsString ** globdb( const EmacsString &key, int *ignore_len );	/* Make a key table */
/*forward*/ static int globdb_helper( const EmacsString &key, unsigned char * *rfs );	/* Make the key table */
#endif
