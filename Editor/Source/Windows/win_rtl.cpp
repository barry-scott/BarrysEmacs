...
CString makeCString( const EmacsString &estr )
{
    return CString( estr.sdata() );
}

EmacsString makeEmacsString( const CString &cstr )
{
    return EmacsString( EmacsString::copy, (unsigned char *)(LPCTSTR)cstr, cstr.GetLength() );
}
...
{
    CString value = theApp.GetProfileString( "DeviceNames", name, "" );
    return makeEmacsString( value );
}
