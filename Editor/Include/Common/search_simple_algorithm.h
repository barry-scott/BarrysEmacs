...
class SearchSimpleAlgorithm : public SearchImplementation
	{
public:
	enum { MAX_NBRA = 64 };		/* the maximum number of meta-brackets in an */
					/* RE -- \( \) */
...
private:
	...
	int cclass( unsigned char *char_set, int c, int af );

	enum { ESIZE = 8192 };		/* the maximum size of an RE */

	unsigned char sea_expbuf[ ESIZE+4 ];		/* The most recently compiled search string */
	unsigned char *sea_alternatives[ MAX_NALTS+1 ];	/* The list of \| seperated alternatives */
