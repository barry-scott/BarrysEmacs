...
namespace Py
	...
	protected:
		...
		static void add_method( const char *name, method_varargs_function_t function, const char *doc="" )
			{
			add_varargs_method( name, function, doc );
			}

		static void add_method( const char *name, method_keyword_function_t function, const char *doc="" )
			{
			add_keyword_method( name, function, doc );
			}
	...
	protected:
		...
		virtual Object getattr_default( const char *_name )
			...
			if( name == "__name__" && type_object()->getName() != NULL )
				{
				return Py::String( type_object()->getName() );
				}
			else if( name == "__doc__" && type_object()->getDoc() != NULL )
				{
				return Py::String( type_object()->getDoc() );
				}
			else
				{
