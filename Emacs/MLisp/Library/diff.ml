...
(defun
	...
	(if (= current-buffer-type "file")
		...
		(save-excursion
		    ...
		    (insert-string "_")
		    (end-of-line)
		    (setq ~backup-filename (region-to-string))
		)
		(save-excursion
		    (save-restriction
			(widen-region)
		...
		(if (| ~buffer-is-not-modified ~prefix)
		    (filter-region (concat diff-command " _diff_.tmp " ~backup-filename))
		    (filter-region (concat diff-command " _diff_.tmp " ~filename))
		)
		(beginning-of-file)
		(unset-mark)
