...
int EmacsString::first( char ch ) const
	{
	// index of ch in string
	const char *p = strchr( sdata(), ch );
	if( p == NULL )
		return -1;
...
int EmacsString::first( unsigned char ch ) const
	{
	return index( (char)ch );
	}

int EmacsString::last( char ch ) const
	{
	// index of ch in string
	if( _rep == 0 )
		return -1;
...
int EmacsString::last( unsigned char ch ) const
	{
	return last( (char)ch );
	}
