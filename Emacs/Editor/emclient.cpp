...
#define debug( msg ) do { if( opt_debug ) {cerr << msg << endl;} } while(0)
//#define debug( msg )

EmacsString realPathToBemacs( const char *argv0 );

ostream &operator<<( ostream &s, const EmacsString &string )
	{
	if( s.opfx() )
		{
...
int main( int argc, char ** argv )
	...
	if( !process_command() )
		...
		if( !do_not_fork )
			{
			cerr << "Starting new bemacs server" << endl;
		...
		if( child == 0 )
			...
			if( i == 0 )
				cerr << "Timed out waiting for the bemacs server to start up" << endl;
			}
		}
...
void make_dir( const EmacsString &final_dir )
	...
	for(;;)
		...
		if( stats.stat( dir ) )
			...
			cerr << "Made missing directory " << dir << endl;
			}
...
void make_fifo( const EmacsString &fifo )
	...
	cerr << "Replacing fifo " << fifo << " contains " << (int)bytes_of_data << " bytes of data." << endl;
	remove( fifo );
