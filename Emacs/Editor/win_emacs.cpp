//theApp-> win_emacs.cpp : Defines the class behaviors for the application.
//
...
bool UI_mod_write( bool ask_before_save )
	...
	for( EmacsBuffer *b = buffers; !err && b != NULL; b = b->b_next )
		{
		b->set_bf();
		...
		&& b->b_modified != 0 )
			...
			while( !save_resolved )
				...
				if( ask_before_save_this_file )
					...
					save_question.m_buffer_name = (const CString &)b->b_buf_name;
					save_question.m_file_name = (const CString &)b->b_fname;
...
cancel:
	...
	// ok if no err and no write error
