...
/*forward*/ void dest_window_ring( EmacsWindowRing *wr );
...
int goto_window_at_x_y( void )
	...
	if( cur_exec == NULL )
		{
		mouse_x = get_number_interactive( "X coordinate: " );	if( err ) return 0;
		mouse_y = get_number_interactive( "Y coordinate: " );	if( err ) return 0;
		}
	else
		{
		if( check_args( 2, 4 ) )
...
void EmacsWindowGroup::do_dsp()
	...
	if( screen_garbaged || err || last_redisplay_paused )
		{
		cant_1win_opt = 1;
...
int EmacsView::dump_line_from_buffer
	...
	if( flags.syntax_colouring )
		...
		bf_cur->syntax_fill_in_array( required );
		}
