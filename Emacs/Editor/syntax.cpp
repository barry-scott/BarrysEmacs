...
/*forward*/ static int scan_comment( int i, SyntaxString *comment );
...
void EmacsBuffer::syntax_fill_in_array( int required )
	{
	syntax_buffer_data *s = &b_syntax;
	enum syn_states state = st_simple;
	int pos;
	struct SyntaxTable::entry *string_entry = NULL;
	...
	for( ; pos<=limit; pos++ )
		...
		case st_simple:
			...
			else if( entry->s_kind&SYNTAX_STRING_MASK )
				{
				set_syntax_at( pos, entry->s_kind );
