...
void EmacsPushButtonOkWidget::button_press_ok_action( EmacsDialog *d )
	...
	if( d->inuse )
		...
		if( !d->apply_keys.isNull() )
			...
			for( EmacsWidget *p = d->children; p && !err; p = p->next )
				p->store();
...
int motif_XmFormDialog( void )
	...
	root_dialog = w;
	exit_widget = NULL;
	for( int i = 5; i <= prg->p_nargs && !err; i++ )
		eval_arg( i );
