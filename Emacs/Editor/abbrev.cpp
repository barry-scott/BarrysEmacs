...
/*forward*/ static int hash_abbrev( const EmacsString &s );
/*forward*/ static AbbrevEntry *lookup_abbrev( AbbrevTable *table, const EmacsString &name, int h );
/*forward*/ static void define_abbrev_phrase( AbbrevTable *table, const EmacsString &abbrev, const EmacsString &phrase, BoundName *proc );
...
/*forward*/ static int read_abbrevs( EmacsString s, int report );
...
int abbrev_expand( void )
	...
	*p = 0;
	...
	while( (n = n - 1) >= 1 && bf_cur->char_at_is( n, SYNTAX_WORD ) )
		{
		p--;
		unsigned char c = bf_cur->char_at( n );
		*p = c;
		if( isupper( c ) )
			{
			uccount++;
	...
	EmacsString phrase( EmacsString::keep, p );
	...
	if( a->abbrev_expansion_hook != NULL )
		...
		bf_cur->del_back( dot, h = &buf[sizeof( buf )] - p - 1 );
		dot_left( h );
		const unsigned char *p = a->abbrev_phrase;
...
int write_abbrev_file( void )
	...
	EmacsFile f;
	f.fio_create( fn, 0, FIO_STD, abbreviations, (FIO_RMS_Attribute)(int)default_rms_attribute );
	if( !f.fio_is_open() )
