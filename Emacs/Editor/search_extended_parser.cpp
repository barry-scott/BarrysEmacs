...
void SearchAdvancedAlgorithm::compile( const EmacsString &pattern, EmacsSearch::sea_type RE )
	{
	if( pattern.isNull() && expression != NULL )
		return;
...
void SearchAdvancedAlgorithm::compile_string( const EmacsString &pattern )
	{
	delete expression;
	expression = new RegularExpressionString( *this, pattern );
...
void SearchAdvancedAlgorithm::compile_expression( const EmacsString &pattern )
	...
	try
		...
		//
		// we claim group 0 for the while match
		// and start hte interior groups at 1
		//
		max_group_number = 1;
		RegularExpressionTerm *term = parse_group_contents( stream );

		expression = new RegularExpressionNumberedGroup( *this, term, 0 );
...
RegularExpressionTerm *SearchAdvancedAlgorithm::parse_term( EmacsStringStream &pattern )
	...
	case '\\':
		...
		|| ch == '0' )
			...
			if( back_ref > max_group_number )
				throw RegularExpressionSyntaxError( FormatString("back reference \\%d invalid, only %d groups") << back_ref << max_group_number );
...
RegularExpressionTerm *SearchAdvancedAlgorithm::parse_group( EmacsStringStream &pattern )
	...
	if( ch == '?' )
		...
		{
		int group_number = max_group_number;
		max_group_number++;
		RegularExpressionTerm *contents = parse_group_contents( pattern );
		term = new RegularExpressionNumberedGroup( *this, contents, group_number );
		}
